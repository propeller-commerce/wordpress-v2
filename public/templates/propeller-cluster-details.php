...
<div class="container-fluid px-0 propeller-product-details propeller-cluster-details <?php apply_filters('propel_product_details_classes', ''); ?>">
    <?php apply_filters('propel_cluster_gecommerce', $this->product, $this->product->defaultProduct, $this); ?>
    ...
    <div class="propeller-product-content" data-id="<?php echo esc_attr($this->product->urlId); ?>" data-slug="<?php echo esc_attr($this->product->slug[0]->value); ?>">
        <div class="row product-gallery-price-wrapper">
            <!-- Product gallery -->
            <div class="col-12 col-lg-7 gallery-wrapper">
                <?php apply_filters('propel_cluster_name_mobile', $this->product); ?>

                <?php apply_filters('propel_cluster_meta_mobile', $this->product, $this->product->defaultProduct, $this); ?>

                <?php apply_filters('propel_cluster_gallery', $this->product->defaultProduct, $this); ?>

                <?php apply_filters('propel_cluster_desc_media', $this->product, $this->product->defaultProduct, $this); ?>
            </div>

            <!-- Product name, pricing, short description -->
            <div class="col-12 col-lg-5">
                <div class="product-price-description-wrapper">
                    <?php apply_filters('propel_cluster_name', $this->product); ?>
                    <?php apply_filters('propel_cluster_meta', $this->product, $this->product->defaultProduct, $this); ?>
                    <?php if (!(!UserController::is_propeller_logged_in() && PROPELLER_WP_SEMICLOSED_PORTAL)) { ?>
                        <?php apply_filters('propel_cluster_price_details', $this->product, $this->product->defaultProduct, $this); ?>
                    <?php } ?>
                    <?php apply_filters('propel_cluster_short_desc', $this->product); ?>
                    <?php if (!(!UserController::is_propeller_logged_in() && PROPELLER_WP_SEMICLOSED_PORTAL)) { ?>
                        <?php apply_filters('propel_product_bulk_prices', $this->product->defaultProduct); ?>
                    <?php } ?>

                    <?php apply_filters('propel_cluster_config', $this->product, $this); ?>
                </div>
            </div>
        </div>
        ...
        <?php apply_filters('propel_cluster_bundles', $this->product->defaultProduct, $this); ?>
