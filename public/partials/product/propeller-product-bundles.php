...
?>
<?php if (count($product->bundles)) { ?>
    <div class="row product-bundles <?php echo esc_html(apply_filters('propel_product_bundles_classes', '')); ?>">
        <div class="col-12">
            <h2 class="product-info-title mt-5 mb-4"><?php echo esc_html(__('Recommended combo deals', 'propeller-ecommerce-v2')); ?></h2>
        </div>
        <div class="col-12">
                        ...
                        <div class="col-12 col-lg-9">
                            <div class="row g-0">
                                ...
                                <?php foreach ($bundle->items as $key => $bundleItem) { ?>
                                    ...
                                    <?php // var_dump($bundleItem->product); die; 
                                    ?>

                                    <div class="<?php echo esc_attr($bundleClass);
                                                if ($bundleItem->isLeader != 'Y') echo ' bundle-item-col'; ?> px-4 pt-2 pb-4">
                                                    ...
                                                    <a href="<?php echo esc_url($obj->buildUrl(PageController::get_slug(PageType::PRODUCT_PAGE), $bundleItem->product->slug[0]->value, $bundleItem->product->urlId)); ?>">
                                                        <?php if ($bundleItem->product->has_images()) { ?>
                                                            <img class="img-fluid"
                                                                ...
                                                                alt="<?php echo esc_attr($bundleItem->product->images[0]->alt[0]->value); ?>"
                                                                loading="lazy"
                                                                width="140"
                                                                height="140">
                                                        <?php } else { ?>
                                                    ...
                                                    <div class="product-price">
                                                        <div class="product-current-price">
                                                            <?php if ($user_prices == false) { ?>
                                                                <span class="price"><span class="symbol"><?php echo esc_html(PropellerHelper::currency()); ?>&nbsp;</span><?php echo esc_html(PropellerHelper::formatPrice($bundleItem->price->gross)); ?><small><?php echo esc_html(__('excl. VAT', 'propeller-ecommerce-v2')); ?></small></span>
                                                            <?php } else { ?>
                                                                <span class="price"><span class="symbol"><?php echo esc_html(PropellerHelper::currency()); ?>&nbsp;</span><?php echo esc_html(PropellerHelper::formatPrice($bundleItem->price->net)); ?><small><?php echo esc_html(__('incl. VAT', 'propeller-ecommerce-v2')); ?></small></span>
                                                            <?php } ?>
                                                        </div>
                                                    </div>
                                                <?php } ?>
                                ...
                                <?php if (!(!UserController::is_propeller_logged_in() && PROPELLER_WP_SEMICLOSED_PORTAL)) { ?>
                                    <div class="add-to-basket">
                                        <div class="bundle-prices">
                                            ...
                                            <?php } else { ?>
                                                <?php if ($bundle->price->originalNet != $bundle->price->net) { ?>
                                                    <div class="bundle-old-price">
                                                        <span class="price"><?php echo esc_html(PropellerHelper::formatPrice($bundle->price->originalNet)); ?></span>
                                                    </div>
                                                <?php } ?>
                                                <div class="bundle-current-price">
                                                    <span class="price"><span class="symbol"><?php echo esc_html(PropellerHelper::currency()); ?>&nbsp;</span><?php echo esc_html(PropellerHelper::formatPrice($bundle->price->net)); ?><small><?php echo esc_html(__('incl. VAT', 'propeller-ecommerce-v2')); ?></small></span>
                                                </div>
                                            <?php } ?>
                                        </div>
                                        <div class="discount-add-to-basket row align-items-center">
                                            <?php if ($user_prices == false) { ?>
                                                ...
                                                if ($bundle->price->originalNet != $bundle->price->net) {
                                                    $bundleDiscount = $bundle->price->originalNet - $bundle->price->net; ?>
                                            ...
                                            <div class="col-12">
                                                <form name="add-to-basket-bundle" method="post" class="add-to-basket-bundle-form">
                                                    <input type="hidden" name="bundle_id" value="<?php echo esc_attr($bundle->id); ?>">
