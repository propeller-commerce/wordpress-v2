...
<div class="col-12 px-0 cluster-config-details cluster-color-options">
    <h3>
    ...
    <?php foreach ($option->options as $config_option) { ?>
        <div class="form-check-inline">
            ...
            <label class="form-check-label <?php echo esc_attr($config_option->disabled); ?>" for="opt_<?php echo esc_attr($config_option->value); ?>">
                <div class="cluster-color <?php echo esc_attr($config_option->disabled); ?> <?php echo esc_html(($option->selected == $config_option->value) || ($count == 1  && empty($config_option->disabled)) ? 'active' : ''); ?>" style="background-color: <?php echo esc_attr($config_option->value); ?>;">&nbsp;</div>
            </label>
        </div>
    <?php } ?>
</div>